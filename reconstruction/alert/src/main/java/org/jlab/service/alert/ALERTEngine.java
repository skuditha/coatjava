...
import org.jlab.rec.alert.AIPID.PrePIDFeatureBuilder;
...
public class ALERTEngine extends ReconstructionEngine {
    ...
    public boolean processDataEvent(DataEvent event) {
        ...
        // ---------------------------------------------------------------------------------------
        // PrePID using AI with the improved ALERT PrePID feature contract.
        //
        // Match resolution follows the shared alert_prepid implementation:
        //   AHDC::track.trackid
        //     -> ALERT::ai:projections.trackid
        //     -> first valid ALERT::ai:projections.matched_atof_hit_id != -1
        //     -> ATOF::hits.id
        //     -> all ATOF::hits rows sharing that hit's clusterid for cluster features.
        //
        // The model consumes one 61-feature row. Standardization is embedded in
        ...
        if (event.hasBank("ALERT::ai:projections") && event.hasBank("AHDC::track") && event.hasBank("ATOF::hits")) {
            ...
            for (int trkRow = 0; trkRow < bankTrk.rows(); trkRow++) {
                int trackid = bankTrk.getInt("trackid", trkRow);
                PrePIDFeatureBuilder.MatchResolution match = PrePIDFeatureBuilder.resolveTrackMatch(bankProj, bankHit, trackid);
                float[] features = PrePIDFeatureBuilder.buildFeatures(bankTrk, trkRow, bankHit, match);

                try {
                    float[] pred = modelPrePID.prediction(features);
                    if (pred == null || pred.length < 6) {
                        continue;
                    }

                    int prepid = (int) pred[0];
                    int atofHitId = match.hasValidMatch ? match.hitId : -1;
                    prepid_results.add(new PrePIDResult(
                            trackid,
                            atofHitId,
                            prepid,
